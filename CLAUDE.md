...
- **D8 (contract chi tiết task + ghi Jira, E5/#10)**: Màn chi tiết dùng lại đúng các handler root-level SẴN CÓ của `qa_dashboard.py` (KHÔNG có prefix `/api/`), app chỉ nói JSON. Contract đọc từ source backend (`core/routes/write.py`, `core/jira_api.py::fetch_issue_detail`, `core/custom_status.py`):
  - **GET `/issue-comments?key=`** → `{ok, detail:{key, summary, description, status, assignee, duedate, updated, created, devs:[…], comments:[{author, when, body}] (mới→cũ), bugs:[{id,summary,severity,status,module}], testcases:[{id,name,count,pass,fail}]}}`; lỗi = 400 `{ok:false,msg}`. Đọc bằng **PAT chung read-only** (không cần PAT cá nhân).
  - **POST ghi Jira bằng PAT cá nhân** (thiếu PAT → 400 `{ok:false, code:"no_pat", msg}`): `/jira-transitions` `{key}`→`{ok, transitions:[{id,to}]}`; `/do-transition` `{key,id}`→`{ok,msg}`; `/add-comment` `{key,body}`→`{ok,msg}`; `/duedate-perm` `{key}`→`{ok,canEdit}` (gate UI, cũng cần PAT); `/set-duedate` `{key,duedate}` (`""`=xoá hạn)→`{ok,msg}`.
  - **POST `/set-custom-status`** `{key,status,summary}` — nhãn overlay nội bộ (KV, **KHÔNG cần PAT**), `status`=1 slug trong 8 nhãn (`""`=gỡ hết), **TOGGLE**, trả `{ok, values:[slug…]}` = list mới; lỗi = 400 `{ok:false}`. Catalog 8 nhãn ở `domain/detail/TaskDetail.kt::CustomStatus` **phải khớp** `core/custom_status.py::CUSTOM_STATUSES` (backend `is_valid` từ chối slug lạ).
  - Mọi write success → app **reload `/issue-comments`** để phản ánh thay đổi. Ghi qua `_reply_json`: **200 khi ok, 400 khi fail** (kèm body JSON) → dùng `Response<T>` đọc body-or-errorBody để lấy `msg`/`code` (như PatApi.savePat). Msg lỗi là tiếng Việt của backend → hiển thị thẳng; fallback hằng số VN trong ViewModel (như PatViewModel).
  - **Detail = network-only** (KHÔNG cache Room): read 1 task nhẹ + phải tươi để thấy write vừa làm.
  - **Custom-status seed**: `/issue-comments` KHÔNG trả `customs` → seed overlay từ `customs` của task lúc bấm (D6/D7 task có `customs`), truyền qua **nav arg** `task/{key}?customs=<csv slug>` (`DetailArgs`). Vì vậy `MyWorkTask`/`DashboardTask` + `TaskDto`/`DashTaskDto` thêm field `customs:List<String>`; MyWork Room thêm cột `customs` (CSV) → **bump `AppDatabase` v3**. Sau mỗi toggle lấy state từ `values` server trả.
- **D9 (contract `/activity-feed` + notification, E7/#11)**: Đọc TRỰC TIẾP từ source backend (`qa_dashboard.py::_get_activity_feed`/`_post_dismiss`, `core/jira_api.py::_compute_activity_feed`, `core/custom_status.py`), KHÔNG có prefix `/api/`.
  - **GET `/activity-feed`** → **200** `{ok, activities:[…], tasks:{key:{customs,status?}}}`, **400 `{ok:false}`** khi Jira không với tới (chuông best-effort). Mỗi activity: `{id, kind, key, summary, author, when, is_unread, old?, new?, assignee?, body?, mention?}`. `id` **ổn định + duy nhất** (`key#…`) = khoá dedup + giá trị `/dismiss`. `kind` ∈ `created/status/priority/duedate/assignee/comment/custom_status`. Backend **đã lọc bỏ activity của chính người xem** (`_drop_own_activities`) + scope theo role (admin=cả team, QA=task họ watch) → app KHÔNG tự lọc self. `tasks` = patch map real-time cho web bell, app **bỏ** (chỉ cần `activities`).
  - **POST `/dismiss`** `{ids:[…]}` → 200 `{ok:true}` / 400 `{ok:false}`. Đánh dấu đã đọc, lưu server-side theo email (đồng bộ chéo máy). Tap noti + swipe đều gọi qua `DismissWorker` (OneTime, retry).
  - **Poll = WorkManager periodic 15'** (KHÔNG phải 60s như spec — periodic tối thiểu của WorkManager là 15', 60s bất khả thi + tốn pin). `@HiltWorker` (cần `androidx.hilt:hilt-work` + `QaApp` là `Configuration.Provider`, tắt default `WorkManagerInitializer` trong manifest). Enqueue khi login, cancel khi logout (drive từ `MainActivity` theo `isLoggedIn`).
  - **Dedup local** `NotifiedStore` (SharedPreferences THƯỜNG — chỉ chứa activity id, KHÔNG secret, OPSEC ok): tránh post lại cùng event mỗi cycle giữa lúc hiện noti và lúc user đọc. Mỗi poll prune về id còn trong feed → bounded. Read-state thật vẫn ở server (`is_unread`/`/dismiss`).
  - **Activity = network-only**, KHÔNG cache Room (ephemeral). Tap noti deep-link vào detail với `customs` rỗng (feed không trả nhãn) → overlay seed rỗng, re-sync từ `values` sau toggle đầu (D8).
